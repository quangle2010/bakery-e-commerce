...
const fetchProductDetail = async () => {
    try {
        const { data } = await axiosClient.get(`/admin/product/${id.value}`);
...
const handleSubmit = async () => {
    ...
    try {
        ...
        const { data } = await axiosClient.put(`/admin/product/${id.value}`, formData, {
            headers: {
...
const setFormData = () => {
    ...
    formSubmit.name = productDetail.value.name || '';
    formSubmit.category = productDetail.value.attributeOptions?.[0] || '';
    formSubmit.flavor = productDetail.value.attributeOptions?.[1] || '';
    formSubmit.image = null;
    formSubmit.price = productDetail.value.price?.toString() || '';
    formSubmit.quantity = productDetail.value.quantity?.toString() || '';
    formSubmit.weight = productDetail.value.weight?.toString() || '';
    formSubmit.description = productDetail.value.description || '';
}
