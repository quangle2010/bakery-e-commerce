  ...
  try {
    ...
    if (response.data.status === true && response.data.data) {
      ...
      const provinceIds = new Set(
        addresses.value.map((addr) => addr.provinceId)
      );
      for (const pid of provinceIds) {
        await fetchDistricts(pid);
      }
      const districtIds = new Set(
        addresses.value.map((addr) => addr.districtId)
      );
...
const confirmAddressPayment = (item: Address) => {
  auth.updateAddressId(item?.id);
  router.push("/payment");
};
...
const defaultItem = async () => {
  ...
  try {
    const response = await axiosClient.put(
      `/user/addresses/${selectedItem.value.id}/default`
    );
    if (response.data.status === true) {
      showSuccess(response.data.message);
      await fetchAddresses();
      auth.updateAddressId(selectedItem.value?.id);
    ...
    if (axios.isAxiosError(error)) {
      ...
      showError(
        error.response?.data.message ||
          "Đã xảy ra lỗi khi đặt làm địa chỉ mặc định"
      );
    } else {
...
const deleteItem = async () => {
  ...
  try {
    const response = await axiosClient.delete(
      `/user/addresses/${selectedItem.value.id}`
    );
    if (response.data.status === true) {
      showSuccess(response.data.message);
    ...
    if (axios.isAxiosError(error)) {
      console.error("API error:", error.response?.data || error.message);
      showError(
        error.response?.data.message || "Đã xảy ra lỗi khi xóa địa chỉ"
      );
    } else {
      console.error("Unexpected error:", error);
...
<template>
  <div class="card mb-4 w-100 h-100">
    <div
      class="d-flex align-items-center justify-content-between p-4 bg-light border-bottom"
    >
      <div class="fw-bold">Chọn địa chỉ nhận hàng</div>
        ...
        >
          <div class="card card_item mb-4">
            ...
            >
              ...
              <div class="d-flex align-items-center gap-2">
                ...
                <button
                  ...
                  @click="confirmAddressPayment(address)"
                >
            ...
            <div class="card-body">
              ...
              <p class="mb-0">
                <strong>Địa chỉ:</strong> {{ addressFullMap.get(address.id) }}
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="modal-content border-0 shadow-sm">
          ...
          <div class="modal-body text-center py-4">
            ...
            <p class="mb-0">
              ...
              <strong>{{ getFullAddress(selectedItem?.id) }}</strong
              >?
            </p>
    ...
    <div
      class="modal fade"
      :class="{ show: showDefaultModal }"
      id="defaultModal"
      tabindex="-1"
      aria-labelledby="defaultModalLabel"
      aria-hidden="true"
      :style="{ display: showDefaultModal ? 'block' : 'none' }"
    >
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-sm">
          <div class="modal-header border-0 pb-0">
          ...
          <div class="modal-body text-center py-4">
            ...
            <p class="mb-0">
              ...
              <strong> {{ getFullAddress(selectedItem?.id) }}</strong> làm địa
              chỉ mặc định?
            </p>
            <p class="text-muted small mt-2">
              Hành động này không thể hoàn tác.
